...
COMPONENT_NAME=ita-demo
SERVICE_USERNAME=ita
SERVICE_ENV=ita-demo.env
...
echo "Setting up ITA Linux User..."
...
if [ "${NOSETUP,,}" == "true" ]; then
    ...
    echo "Update systemd configuration with ITA details for manual startup"
    echo "Installation completed successfully!"
else
    # create config.json
    CONFIG_JSON=$PRODUCT_HOME/config.json
    echo "{\"trustauthority_api_url\": \"$ITA_API_URL\"," > $CONFIG_JSON
    echo " \"trustauthority_api_key\": \"$ITA_API_KEY\"}" >> $CONFIG_JSON
    ...
    echo "Environment=ITA_API_URL=$ITA_API_URL" >> $SYSD_CONF
    echo "Environment=ITA_API_KEY=$ITA_API_KEY" >> $SYSD_CONF
